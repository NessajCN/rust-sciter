...
impl Value {
	...
	fn ensure_tmp(&mut self) -> Box<Value> {
		match self.tmp {
			Some(ref p) => p.clone(),
			None => Box::new(Value::new()),
		}
		// if self.tmp.is_null() {
	...
	pub const fn is_primitive(&self) -> bool {
		...
		match self.data.t {
			T_UNDEFINED | T_NULL | T_BOOL | T_INT | T_FLOAT | T_DATE => true,
			_ => false,
		}
	}
...
impl ::std::ops::Index<usize> for Value {
	...
	fn index(&self, index: usize) -> &Self::Output {
		match index {
			0 => &self,
			i => self.next_value().expect("index out of bound").index(i - 1)
		}
